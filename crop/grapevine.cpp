...
Vintage_Grapevine::Vintage_Grapevine() {}

bool Vintage_Grapevine::compute(bool computeDaily, int secondsPerStep, TvineField* vineField, double chlorophyll)
{
    ...
    //PHENOLOGY
    this->statePlant.stateGrowth.tartaricAcid =  Vintage_Grapevine::getTartaricAcid();
    Vintage_Grapevine::getPotentialBrix();

    if (isVegetativeSeason)
    {
        if (computeDaily)
        {
            Vintage_Grapevine::getLAIVine(vineField);
        }
        //Vintage_Grapevine::photosynthesisRadiationUseEfficiency(); // da usare solo quando non funziona Farquhar
        Vintage_Grapevine::photosynthesisAndTranspiration(vineField->cultivar);
...
void Vintage_Grapevine::initializeRootProperties (soil::Crit3DSoil* mySoil,
                                                  ...
                                                  rootDistributionType type, double depthMode, double depthMean)
{
    ...
    layerRootDensity =  Vintage_Grapevine::rootDensity(mySoil, soilLayersNr, soilLayerWithRoot,
...
void Vintage_Grapevine::setDate (Crit3DTime myTime)
{
...
bool Vintage_Grapevine::setWeather(double meanDailyTemp, double temp, double irradiance , double prec , double relativeHumidity , double windSpeed, double atmosphericPressure)
{
...
bool Vintage_Grapevine::setDerivedVariables ( double diffuseIrradiance, double directIrradiance, double transmissivity, double sunElevation, float clearSkyTransmissivity, double etp)
{
    bool isReadingOK = false;
...
void Vintage_Grapevine::initializeWaterStress(Tvine* cultivar)
{
    for (int i = 0; i < soilLayersNr; i++)
    {
...
bool Vintage_Grapevine::setSoilProfile(double* myWiltingPoint, double* myFieldCapacity,
...
bool Vintage_Grapevine::setStatePlant(TstatePlant myStatePlant, bool isVineyard)
{
...
TstatePlant Vintage_Grapevine::getStatePlant()
{
    return(this->statePlant);
}

ToutputPlant Vintage_Grapevine::getOutputPlant()
{
    return (this->statePlant.outputPlant);
}


void Vintage_Grapevine::getFixSimulationParameters()
{
...
bool Vintage_Grapevine::initializeStatePlant(int doy, TvineField* vineField)
{
    getFixSimulationParameters();
...
bool Vintage_Grapevine::fieldBookAction(TvineField* vineField, TfieldOperation action, float quantity)
{
...
void Vintage_Grapevine::photosynthesisRadiationUseEfficiency(Tvine* cultivar)
{
...
void Vintage_Grapevine::photosynthesisAndTranspiration(Tvine* cultivar)
{
    Vintage_Grapevine::weatherVariables();
    Vintage_Grapevine::radiationAbsorption();
    //Vintage_Grapevine::leafTemperature();
    Vintage_Grapevine::aerodynamicalCoupling();
    Vintage_Grapevine::upscale(cultivar);
    Vintage_Grapevine::carbonWaterFluxesProfileNoStress(cultivar);
    Vintage_Grapevine::carbonWaterFluxesProfile(cultivar);
    Vintage_Grapevine::cumulatedResults();
    Vintage_Grapevine::getStressCoefficient();

}

double Vintage_Grapevine::getCO2()
{
...
double Vintage_Grapevine::acclimationFunction(double Ha , double Hd, double leafTemp,
...
double Vintage_Grapevine::acclimationFunction2(double preFactor, double expFactor,
...
void Vintage_Grapevine::weatherVariables()
{
    // taken from Hydrall Model, Magnani UNIBO
...
void Vintage_Grapevine::radiationAbsorption()
{
    // taken from Hydrall Model, Magnani UNIBO
...
void Vintage_Grapevine::leafTemperature()
{
    // taken from Hydrall Model, Magnani UNIBO
    ...
    double  diffuseLightSector1K,diffuseLightSector2K,diffuseLightSector3K ;
    //projection of the unit leaf area in the direction of the sun's beam, following Sellers 1985 (in Wang & Leuning 1998)
    double sineSolarElevation = maxValue(0.001,sin(mySunElevation*DEG_TO_RAD));
...
void Vintage_Grapevine::aerodynamicalCoupling()
{
        // taken from Hydrall Model, Magnani UNIBO
        ...
        // stability factor F (=1/FIM/FIH)
            ...
            moninObukhovLength = -(pow(frictionVelocity,3))*CP*myAtmosphericPressure;
            ...
            radiativeConductance= 4*(slopeSatVapPressureVSTemp/GAMMA)*(STEFAN_BOLTZMANN/CP)*pow((myInstantTemp + ZEROCELSIUS),3);
...
void Vintage_Grapevine::upscale(Tvine *cultivar)
{
    // taken from Hydrall Model, Magnani UNIBO
    double sineSolarElevation = maxValue(0.0001,sin(mySunElevation*DEG_TO_RAD));
...
void Vintage_Grapevine::photosynthesisKernel(Tvine* cultivar, double COMP,double GAC,double GHR,double GSCD,double J,double KC,double KO
      ...
      if (J >= 1.0e-7)
      {
            ...
            while ((I++ < Imax) && (deltaAssimilation > myTolerance))
            {
                ...
                *GSC = maxValue(*GSC,1.0e-5);
                ...
                VPDS = (slopeSatVapPressureVSTemp/CP*RNI + vaporPressureDeficit*GHR) / (GHR+(*GSC)*DUM1);  //VPD at the leaf surface (Pa)
...
void Vintage_Grapevine::photosynthesisKernelSimplified(Tvine* cultivar, double COMP,double GSCD,double J,double KC,double KO
...
void Vintage_Grapevine::carbonWaterFluxes(Tvine* cultivar)
    {
        // taken from Hydrall Model, Magnani UNIBO
        ...
        // shaded big leaf
        Vintage_Grapevine::photosynthesisKernelSimplified(cultivar, shaded.compensationPoint,shaded.minimalStomatalConductance,shaded.maximalElectronTrasportRate,shaded.carbonMichaelisMentenConstant,shaded.oxygenMichaelisMentenConstant,shaded.darkRespiration,alphaLeuning,shaded.maximalCarboxylationRate,&shaded.assimilation,&shaded.stomatalConductance,&shaded.transpiration);
...
void Vintage_Grapevine::carbonWaterFluxesProfile(Tvine* cultivar)
    {
        // taken from Hydrall Model, Magnani UNIBO
        assimilationInstant = 0 ;
        ...
        for (int i=0;i<soilLayersNr;i++)
        {
            ...
            if(sunlit.leafAreaIndex > 0)
            {
                Vintage_Grapevine::photosynthesisKernelSimplified(cultivar, sunlit.compensationPoint,sunlit.minimalStomatalConductance,
            ...
            // shaded big leaf
            Vintage_Grapevine::photosynthesisKernelSimplified(cultivar, shaded.compensationPoint,shaded.minimalStomatalConductance,
...
void Vintage_Grapevine::carbonWaterFluxesProfileNoStress(Tvine* cultivar)
    {
        // taken from Hydrall Model, Magnani UNIBO
...
double Vintage_Grapevine::getStressCoefficient()
{
...
void Vintage_Grapevine::cumulatedResults()
{
    // taken from Hydrall Model, Magnani UNIBO
...
double Vintage_Grapevine::plantRespiration()
{
    // taken from Hydrall Model, Magnani UNIBO
    ...
    //leafRespiration *= maxValue(0,minValue(1,Vintage_Grapevine::temperatureMoistureFunction(myInstantTemp + ZEROCELSIUS))) ;
    //sapwoodRespiration *= maxValue(0,minValue(1,Vintage_Grapevine::temperatureMoistureFunction(myInstantTemp + ZEROCELSIUS))) ;
    //shootRespiration *= maxValue(0,minValue(1,Vintage_Grapevine::temperatureMoistureFunction(myInstantTemp + ZEROCELSIUS))) ;
    mySoilTemp = Vintage_Grapevine::soilTemperatureModel();
    rootRespiration *= maxValue(0,minValue(1,Vintage_Grapevine::temperatureMoistureFunction(mySoilTemp + ZEROCELSIUS))) ;
...
double Vintage_Grapevine::soilTemperatureModel()
{
    // taken from Hydrall Model, Magnani UNIBO
    double temp;
...
double Vintage_Grapevine::temperatureMoistureFunction(double temperature)
{
    // taken from Hydrall Model, Magnani UNIBO
    int   MODEL;
...
void Vintage_Grapevine::plantInterception(double fieldCoverByPlant)
{
    // taken from Hydrall Model, Magnani UNIBO
...
double Vintage_Grapevine::meanLastMonthTemperature(double previousLastMonthTemp)
{
...
double* Vintage_Grapevine::rootDensity(soil::Crit3DSoil* mySoil, int nrLayers,
                                       int nrLayersWithRoot, int nrUpperLayersWithoutRoot,
                                       rootDistributionType type, double* layerDepth,
...
bool Vintage_Grapevine::getExtractedWater(double* myWaterExtractionProfile)
{
    for(int i=0; i<soilLayersNr; i++)
    {
...
double Vintage_Grapevine::getRealTranspirationGrapevine()
{
    double sum = 0.0;
    for(int i=0; i<soilLayersNr; i++)
    {
...
double Vintage_Grapevine::getRealTranspirationGrass()
{
    double sum = 0.0;
    for(int i=0; i<soilLayersNr; i++)
    {
...
bool Vintage_Grapevine::getExtractedWaterFromGrassTranspirationandEvaporation(double* myWaterExtractionProfile)
{
...
double Vintage_Grapevine::chillingRate(double temp, double aParameter, double cParameter)
{
    return (2./(1+exp(aParameter*(temp-cParameter)*(temp-cParameter))));
}

double Vintage_Grapevine::criticalForceState(double chillingState,double co1 , double co2)
{
    return co1*exp(co2*chillingState);
}

double Vintage_Grapevine::forceStateFunction(double forceState , double temp, double degDaysVeraison)
{
...
double Vintage_Grapevine::forceStateFunction(double forceState , double temp)
{
    forceState += (1./(1+exp(-0.26*(temp-16.06))));
    return forceState;
}
void Vintage_Grapevine::printResults(std::string fileName, bool* isFirst, Tvine* cultivar)
{
    FILE *fp;
    if (*isFirst)
    {
...
void Vintage_Grapevine::computePhenology(bool computeDaily, bool* isVegSeason, Tvine* cultivar)
{
    ...
    if (computeDaily)
    {
        ...
        if (this->statePlant.statePheno.stage != NOT_INITIALIZED_VINE)
            this->statePlant.statePheno.stage = float(endoDormancy + minValue(1,this->statePlant.statePheno.chillingState / cultivar->parameterPhenoVitis.criticalChilling));
        ...
        if ((this->statePlant.statePheno.chillingState > cultivar->parameterPhenoVitis.criticalChilling))
        {
            ...
            this->statePlant.statePheno.stage = ecoDormancy + minValue(1, 1 - float((criticalForceStateBudBurst-this->statePlant.statePheno.forceStateBudBurst)/criticalForceStateBudBurst));
        }
            ...
            *isVegSeason = true;
            ...
            if (this->statePlant.statePheno.forceStateVegetativeSeason > cultivar->parameterPhenoVitis.criticalForceStateVeraison)
            {
                this->statePlant.statePheno.stage = veraison +
                      float((this->statePlant.statePheno.forceStateVegetativeSeason-cultivar->parameterPhenoVitis.criticalForceStateVeraison) /
            ...
            else if (this->statePlant.statePheno.forceStateVegetativeSeason > cultivar->parameterPhenoVitis.criticalForceStateFruitSet)
            {
                if (this->statePlant.statePheno.degreeDaysAtFruitSet == NODATA)
                    ...
                    this->statePlant.statePheno.stage = fruitSet +
                        float((this->statePlant.statePheno.degreeDaysFromFirstMarch - this->statePlant.statePheno.degreeDaysAtFruitSet) /
            ...
            else if (this->statePlant.statePheno.forceStateVegetativeSeason > cultivar->parameterPhenoVitis.criticalForceStateFlowering)
            {
                this->statePlant.statePheno.stage = flowering +
                        float((this->statePlant.statePheno.forceStateVegetativeSeason - cultivar->parameterPhenoVitis.criticalForceStateFlowering) /
...
double  Vintage_Grapevine::leafWidth()
{
...
double Vintage_Grapevine::getWaterStressByPsiSoil(double myPsiSoil,double psiSoilStressParameter,double exponentialFactorForPsiRatio)
{
...
double Vintage_Grapevine::getWaterStressSawFunction(int index, Tvine* cultivar)
{
...
double Vintage_Grapevine::getWaterStressSawFunctionAverage(Tvine* cultivar)
{
...
//incolto
double Vintage_Grapevine::getFallowTranspiration(double stress, double lai, double sensitivityToVPD)
{
    if (lai <= 0) return 0.0;
...
double Vintage_Grapevine::getGrassTranspiration(double stress, double laiGrassMax, double sensitivityToVPD, double fieldCoverByPlant)
{
...
double Vintage_Grapevine::getGrassRootDensity(int layer, float startRootDepth, float totalRootDepth)
{
    float upperDepth = float(layerDepth[layer] - layerThickness[layer]*0.5);
    float lowerDepth = float(layerDepth[layer] + layerThickness[layer]*0.5);
    if (upperDepth > totalRootDepth) return 0.0;
    if (lowerDepth < startRootDepth) return 0.0;

    float x1 = maxValue(startRootDepth, upperDepth);
    float x2 = minValue(totalRootDepth, lowerDepth);
    float m = float(-2.0) / (totalRootDepth*totalRootDepth);
    float q = float(2.0) / totalRootDepth;
    float y1 = m*x1 + q;
    float y2 = m*x2 + q;
    //trapezoid
...
//----------------------------------------------------------------------
void Vintage_Grapevine::fallowTranspiration(double lai, double sensitivityToVPD)
{
...
void Vintage_Grapevine::grassTranspiration(double laiGrassMax, double sensitivityToVPD, double fieldCoverByPlant)
{
    deltaTime.transpirationGrass = 0.;
    double transpiration, rootDensity;
    for (int layer=0; layer < soilLayersNr; layer++)
...
double Vintage_Grapevine::getLaiStressCoefficient()
{
    double stress;
...
double Vintage_Grapevine::getLAIGrass(bool isShadow, double laiGrassMax)
{
...
void Vintage_Grapevine::getLAIVine(TvineField* vineField)
{
    double shootLeafArea;
    ...
    if (laiOld != NODATA)
    {
        ...
        deltaLai *= Vintage_Grapevine::getLaiStressCoefficient();
...
void Vintage_Grapevine::getPotentialBrix()
{
    //potentialBrix = 28;
...
double Vintage_Grapevine::getTartaricAcid()
{
...
double Vintage_Grapevine::gompertzDistribution(double stage)
{
    double a,b,c,output;
...
void Vintage_Grapevine::fruitBiomassRateDecreaseDueToRainfall()
{
    double reduction = 0;
    if (myPrec > 0)
    {
...
double Vintage_Grapevine::getRootDensity(int myLayer)
{
