...
int main(int argc, char *argv[])
{
    ...
    // iterate input files on climate (name of climate.csv = name of observed.csv = name of .xml)
    ...
    for (int i = 0; i < fileList.size(); ++i)
    {
        ...
        if (isOk)
        {
            ...
            // read climate data from csv
            if ( !readCsv(climateFileName, separator, NODATA, &climateDailyObsData) )
                return false;

            // read observed data (at least last 9 months) from csv
            if ( !readCsv(observedFileName, separator, NODATA, &lastYearDailyObsData) )
                return false;
            ...
            //qDebug() << "\nClimate first date = " << getDateString(climateObsFirstDate);
            ...
            //qDebug() << "Climate last date = " << getDateString(climateObsLastDate);
            ...
            if (! makeScenario(outputFileName, separator, &XMLAnomaly, wGenClimate, &lastYearDailyObsData, XMLAnomaly.repetitions, XMLAnomaly.anomalyYear, wgDoy1, wgDoy2, thresholdPrec))
                qDebug() << "\nComputation FAILED";
        }
    }
