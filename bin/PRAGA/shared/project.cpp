...
/*
bool Crit3DProject::aggregateAndSaveDailyMap(meteoVariable myVar, aggregationMethod myAggregation, const QDate& myDate,
                                            const QString& dailyPath, const QString& hourlyPath)
{
    ...
    {
        if (gis::readEsriGrid((hourlyPath + getOutputNameHourly(myVar, myTime)).toStdString(), myMap, &myError))
        {
            if (myTime == myTimeIni)
            {
                for (row = 0; row < myAggrMap->header->nrRows; row++)
                    for (myCol = 0; myCol < myAggrMap->header->nrCols; myCol++)
                        myAggrMap->value[row][myCol] = myMap->value[row][myCol];
    ...
    QString filename = getOutputNameDaily(varName,  "", qDate);
