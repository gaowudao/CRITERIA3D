...
bool Crit3DMeteoPoint::setMeteoPointCurrentValueD(const Crit3DDate& myDate, meteoVariable myVar)
{
    long i = obsDataD[0].date.daysTo(myDate);
    if ((i <0) || (i >= nrObsDataDaysD)) return false;

    if (myVar == dailyAirTemperatureMax)
        currentValue = obsDataD[i].tMax;
    else if (myVar == dailyAirTemperatureMin)
        currentValue = obsDataD[i].tMin;
    else if (myVar == dailyAirTemperatureAvg)
        currentValue = obsDataD[i].tAvg;
    else if (myVar == dailyPrecipitation)
        currentValue = obsDataD[i].prec;
    else if (myVar == dailyAirHumidityMax)
        currentValue = obsDataD[i].rhMax;
    else if (myVar == dailyAirHumidityMin)
        currentValue = obsDataD[i].rhMin;
    else if (myVar == dailyAirHumidityAvg)
        currentValue = obsDataD[i].rhAvg;
    else if (myVar == dailyGlobalRadiation)
        currentValue = obsDataD[i].globRad;
    else if (myVar == dailyReferenceEvapotranspiration)
        currentValue = obsDataD[i].et0;
    else if (myVar == dailyWindIntensityAvg)
        currentValue = obsDataD[i].windIntAvg;
    else if (myVar == dailyWindDirectionPrevailing)
        currentValue = obsDataD[i].windDirPrev;
    else if (myVar == dailyWindIntensityMax)
        currentValue = obsDataD[i].windIntMax;
    else if (myVar == dailyLeafWetness)
        currentValue = obsDataD[i].leafW;
    else if (myVar == dailyWaterTableDepth)
        currentValue = obsDataD[i].waterTable;
    else
        return false;
...
bool Crit3DMeteoPoint::setMeteoPointCurrentValueH(const Crit3DDate& myDate, int myHour, int myMinutes, meteoVariable myVar)
{
    // check indexes (day, hour)
    int i = obsDataH[0].date.daysTo(myDate);
    if ((i < 0) || (i >= nrObsDataDaysH)) return false;
    ...
    if (myVar == airTemperature)
        currentValue = obsDataH[i].tAir[h];
    else if (myVar == precipitation)
        currentValue = obsDataH[i].prec[h];
    else if (myVar == airHumidity)
        currentValue = obsDataH[i].rhAir[h];
    else if (myVar == airDewTemperature)
        currentValue = obsDataH[i].tDew[h];
    else if (myVar == globalIrradiance)
        currentValue = obsDataH[i].irradiance[h];
    else if (myVar == referenceEvapotranspiration)
        currentValue = obsDataH[i].et0[h];
    else if (myVar == windIntensity)
        currentValue = obsDataH[i].windInt[h];
    else if (myVar == leafWetness)
        currentValue = obsDataH[i].leafW[h];
    else if (myVar == atmTransmissivity)
        currentValue = obsDataH[i].transmissivity[h];
    else
        return false;
