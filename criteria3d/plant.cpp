...
bool passPlantTranspirationProfileToMap(long row, long col, Crit3DProject* myProject)
{
    ...
    for (int layer=0; layer<(myProject->nrSoilLayers-1); layer++)
    {
        myProject->outputPlantMaps->transpirationLayerMaps[layer]->value[row][col] = myProject->currentProfile[layer];        
    }
    return true;
}
...
bool loadPlantState(Crit3DProject* myProject, plantVariable myVar, QDate myDate, QString myPath, QString myArea)
{
    QString varName = getVarNameFromPlantVariable(myVar);
    QString fileName = myPath + myDate.toString("yyyyMMdd_") + myArea + "_" + varName;
